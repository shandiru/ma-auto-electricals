...
export const createCheckoutSession = async (req, res) => {
  ...
  try {
    ...
    const line_items = cart.map((item) => {
      const unitAmount = Math.round(Number(item.price) * 100); // in pence
      ...
      return {
        price_data: {
          ...
          product_data: {
            name: item.name,
            description: item.description?.substring(0, 200), // truncate
          },
    ...
    // Compact cart for metadata (to avoid >500 chars)
...
export const checkoutSuccess = async (req, res) => {
  ...
  try {
    ...
    // ✅ Send email to user
    const userSubject = `Your Order Confirmation - ${orderId}`;
    const userHtml = `
